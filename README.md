...
path_quadratura = os.path.abspath(os.path.join(os.path.dirname(**file**), ".."))
sys.path.append(path_quadratura)

from quadratura.quadratura_backend import quadratura

def diamond_difference( sigmaT, sigmaS0, sigmaS1, sigmaS2, Qj, NiSigmaF, N, cce,
ccd, precisao, NN, regioes, n_regioes, esp_R, ): iteracao = 0 # Iniciando as
iterações mi, w = quadratura(N) # Mis e Omegas da quadratura pt = sum(NN) + 1 #
Número de pontos totais NNT = sum(NN) # Número de nodos totais
